...
You can find these reference files in the directory `Data/references`.
...
cellranger mkref \
  ...
  --genes "PATH TO ANNOTATION GTF FILE" \
  --genome "NAME FOR OUTPUT FOLDER" \
  --nthreads "NUMBER OF CPUS"
```

where:

* `--fasta` is the path to a file containing the reference genome in FASTA format   
* `--genes` is the path to a file containing the transcript annotation file in GTF format
* `--genome` is a name for the output folder where cellranger will save the genome index (you do not need to create this folder)
* `--nthreads` is the number of CPUs CellRanger can use, which may speed the indexing (up to a point).
...
In this exercise we will prepare a reference for Cell Ranger using the `mkref` tool.
We index chromosome 21 of the human genome, which is sufficient for the purposes of demonstrating how this step works. 
In a real-life scenario you would index the full genome.

Open a text editor (e.g. Visual Studio Code from the toolbar or `nano` from the terminal) and create a shell script named `01_prepare_reference.sh` to index the reference genome for Cell Ranger. 

- You'll need to give CellRanger the path to the reference genome and gene annotation files found in `Data/references`. 
- Set the output genome index as `GRCh38_chr21_index`.
- Use 7 CPUs for the indexing.

The start of the script is given here, so you can copy this to a new file and work from there. 
You'll need to replace the **"YOUR CODE HERE"** with the necessary arguments for the `cellranger mkref` command.
...
Once ready, we can run our script from the terminal with:
...
cellranger count --id "OUTPUT_SAMPLE_NAME" \
                 --transcriptome "DIRECTORY_WITH_REFERENCE_INDEX" \
...
"ETV6_RUNX1" cancer subtype, and are located in `Data/fastq`. 

* Run `cellranger count` using the fastq files for this sample and have Cell
Ranger name the output directory "ETV6_RUNX1_rep1".
* You will also need to give Cell Ranger the path to the cell_ranger index that
we just generated.
  * **Note**: If you didn't complete the previous exercise, you can use a pre-processed genome index provided in `Data/references/cellranger_index`.
...
Once your code is complete, run your script from the terminal using `bash`. 
...
See the example above for the `--transcriptome` option and see if you can adapt it also for specifying the path to the FASTQ folder.
...
We can run the script from the terminal with:
