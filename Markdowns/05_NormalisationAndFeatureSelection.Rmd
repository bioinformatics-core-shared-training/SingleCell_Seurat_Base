...
- We also import specific functions from the `matrixStats` package for calculating summary statistics from matrices
...
```{r norm_intro_plots}
one_sam_tab <- seurat_object@meta.data %>%
  filter(orig.ident == "PBMMC-1") %>%
  rownames_to_column("Cell") %>%
  dplyr::select(orig.ident, Cell, nCount_RNA) %>%
  mutate(cell_num = seq_len(n()))

p_before_norm <- ggplot(data = one_sam_tab, aes(x = cell_num, y = nCount_RNA)) +
  ...
  theme_classic() +
  theme(
    plot.title = element_text(hjust = 0.5, size = 20, color = "firebrick")
  )
...
```{r HVGs}
seurat_object_norm <- FindVariableFeatures(
  seurat_object_norm,
...
seurat_object_norm <- FindVariableFeatures(
  seurat_object_norm,
  selection.method = "vst",
  nfeatures = round(0.1 * nrow(seurat_object_norm))
)
VariableFeaturePlot(seurat_object_norm)
```
...
seurat_object_norm <- ScaleData(
  seurat_object_norm,
  features = rownames(seurat_object_norm),
  verbose = FALSE
)
...
ggplot(mean_var_df, aes(x = mean, y = variance)) +
  ...
  geom_line(aes(y = trendline), color = "red", linewidth = 1) +
  scale_x_log10() +
  scale_y_log10() +
...
top_3000 <- head(VariableFeatures(seurat_object_norm), 3000)
mean_var_df <- mean_var_df %>%
  mutate(top_3000 = ifelse(
    rownames(mean_var_df) %in% top_3000,
    TRUE,
    FALSE
  ))
ggplot(mean_var_df, aes(x = mean, y = variance)) +
  geom_point(alpha = 0.3, aes(colour = factor(top_3000))) +
  geom_line(aes(y = trendline), color = "black", linewidth = 1) +
  scale_x_log10() +
  scale_y_log10() +
  labs(
...
raw_tab <- seurat_pbmmc1@assays$RNA$counts %>%
  as.data.frame() %>%
  bind_rows(summarise_all(., ~ if (is.numeric(.)) sum(.)))
raw_count <- as.data.frame(t(raw_tab[29787, ])) %>%
  mutate(cell_num = seq_len(n()))

sct_tab <- seurat_pbmmc1@assays$SCT$counts %>%
  as.data.frame() %>%
  bind_rows(summarise_all(., ~ if (is.numeric(.)) sum(.)))
sct_count <- as.data.frame(t(sct_tab[23602, ])) %>%
  mutate(cell_num = seq_len(n()))

p_before_norm <- ggplot(
  data = raw_count,
  aes(x = cell_num, y = ...29787)
) +
  geom_bar(stat = "identity") +
  labs(
    x = "Cell Index",
