...
DimPlot(seurat_object, reduction = "pca", dims = c(2, 3))
...
DimPlot(seurat_object, reduction = "pca", split.by = "SampleGroup")
...
Notice by deafult it colours using the `orig.ident` column in the meta.data. 
...
1. Add the t-SNE result to the reducedDim slot of the seurat object. 
2. Name this reducedDim "TSNE_perplex30". Check the documentation for `RunTSNE`
to help you do this.
3. Set perplexity = 30 (which is the default if we don't specify it). 
4. Run t-SNE based on the PCA we ran previously using the first 10 principal
components. 
5. Plot the TSNE results using the DimPlot and FeaturePlot functions.

We can adjust the perplexity parameter by adding the `perplexity` option.
...
seurat_object <- RunTSNE(seurat_object, 
                         ...
                         reduction.key = "TSNEperplex30seed321_")
DimPlot(seurat_object, reduction = "TSNE_perplex30_seed321")
```
...
seurat_object <- RunTSNE(seurat_object, 
                         ...
                         reduction.key = "TSNEperplex500_")

```

Then visualise using `DimPlot`

```{r answer1c2, eval=FALSE}
...
```{r answer1d, eval=FALSE}
# colour tsne by CD79A expression
FeaturePlot(seurat_object, features = "CD79A", reduction = "TSNE_perplex30")
```
...
Run the UMAP with 5 and 500 neighbours and compare the results. Similarly tothe TSNE, running with higher neighbour values will take more time.
