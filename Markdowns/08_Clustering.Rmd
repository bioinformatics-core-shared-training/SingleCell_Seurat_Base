...
```{r silhouettePlots}
# beeswarm
plot_sil_beeswarm <- function(sil_dat) {
  sil_tab <- sil_dat %>%
    as.data.frame() %>%
    mutate(closestCluster = ifelse(sil_width > 0, cluster, neighbor) %>%
             factor())
  ...
  plt <- scater:::.resolve_plot_colours(plt, sil_tab$closestCluster,
                                        "closestCluster",
                                        colour = TRUE)
  plt
}

p1 <- plot_sil_beeswarm(silhouette_width)
...
silhouette_width_leiden <- silhouette(as.numeric(clusters_leiden),
                                      dist(pca_data))
p1_leiden <- plot_sil_beeswarm(silhouette_width_leiden)
...
for (k in k_values) {
  ...
  seurat_object <- FindClusters(seurat_object,
                                ...
                                algorithm = 4,
                                random.seed = 123,
                                cluster.name = paste0("Leiden_k", k,
                                                      "_res0.8"))
}
...
for (res in res_values) {
  seurat_object <- FindClusters(seurat_object,
                                ...
                                algorithm = 4,
                                random.seed = 123,
                                cluster.name = paste0("Leiden_k17_res",
                                                      res))
}
