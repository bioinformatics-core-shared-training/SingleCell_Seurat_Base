...
# load and filter for only ETV6 and PBMMC samples
seurat_object <- readRDS("../RObjects/Annotated.500.rds")
seurat_object <- subset(seurat_object, subset = SampleGroup == "ETV6RUNX1" | SampleGroup == "PBMMC")
```
...
This looks the same as our UMAP plot from the Seurat object, so we can be confident that the conversion was successful. We can now use this SCE object for downstream analysis using Bioconductor packages.